...
<form name="search" id="search" action="[{ $shop->selflink }]" method="post">
    [{ $shop->hiddensid }]
    <input type="hidden" name="cl" value="manufacturer_list">
...
<tr class="listitem">
    <td valign="top" class="listfilter first">
        ...
        <input class="listedit" type="text" size="50" maxlength="128" name="where[[{$listTable}].oxtitle]" value="[{ $where->oxmanufacturers__oxtitle }]">
        </div></div>
    </td>
    <td valign="top" class="listfilter" height="20" colspan="2">
        ...
        <input class="listedit" type="text" size="50" maxlength="128" name="where[[{$listTable}].oxshortdesc]" value="[{ $where->oxmanufacturers__oxshortdesc }]">
        </div></div>
    </td>
</tr>
<tr>
...
[{foreach from=$mylist item=listitem}]
    ...
    [{ /if}]
    <td valign="top" class="[{ $listclass}]" height="15"><div class="listitemfloating">&nbsp;<a href="Javascript:EditThis('[{ $listitem->oxmanufacturers__oxid->value}]');" class="[{ $listclass}]">[{ $listitem->oxmanufacturers__oxtitle->value }]</a></div></td>
    <td valign="top" class="[{ $listclass}]" height="15"><div class="listitemfloating"><a href="Javascript:EditThis('[{ $listitem->oxmanufacturers__oxid->value}]');" class="[{ $listclass}]">[{ $listitem->oxmanufacturers__oxshortdesc->value }]</a></div></td>
    <td class="[{ $listclass}]" align="right">
      [{if !$readonly}]
          <a href="Javascript:DeleteThis('[{ $listitem->oxmanufacturers__oxid->value }]');" class="delete" id="del.[{$_cnt}]" title="" [{include file="help.tpl" helpid=item_delete}]></a>
      [{/if}]
    </td>
</tr>
[{if $blWhite == "2"}]
